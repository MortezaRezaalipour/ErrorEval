...
while(not foundWCE):
	...
	stats['et'] = (upper_bound + lower_bound) // 2
	...
	if response == unsat:
		...
		if upper_bound - lower_bound <= 1:
			foundWCE = True
			stats['wce'] = lower_bound
		else:
			upper_bound = stats['et']
		stats['num_unsats'] += 1
		stats['unsat_runtime'] += (end_iteration - start_iteration)
end_whole = time.time()
with open('output/report/abs_diff_i4_o3_single_gate_removal_bisection/abs_diff_i4_o3_single_gate_removal_bisection_g19.csv', 'w') as f:
